...
To use **Dapper.Transaction**, we need to create a database first, and then we will perform various database operations using **Dapper.Transaction**.
...
CREATE DATABASE [BookStoreDapperTrans]

GO

USE [BookStoreDapperTrans]
...
It will create a `BookStoreDapperTrans` database that contains two tables. Now we need to insert some data into these tables, which can be used later. 
